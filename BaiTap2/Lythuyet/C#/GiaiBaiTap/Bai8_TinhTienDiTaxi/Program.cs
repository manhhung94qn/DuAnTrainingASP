...
namespace Bai8_TinhTienDiTaxi
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            inputDistance:
            distancePre = Console.ReadLine();
            if (!Ft.isNumber(distancePre))
            {
                Console.Write("Vui long nhap khoang cach la mot so: ");
                goto inputDistance;
            }

            distance = float.Parse(distancePre);
            if (distance > 1)
            {
